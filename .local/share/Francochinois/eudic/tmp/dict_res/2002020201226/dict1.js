...
var image_path="";
function collinsTitleChange(block) { var span = youdaoEl(block+"_t");if(span){var t = span.innerHTML;if(t=="+"){span.innerHTML="-";}else{span.innerHTML="+";}}}
function showCollinsContent(block) { var span = youdaoEl(block+"_t");if(span){span.innerHTML="-";}showDefinition(block);}
function showSpecSent() { var ss = document.all('special_sent'); if (!ss.length) { ss.show(); } for (i = 0; i < ss.length; i++) { ss[i].show(); } var btn = youdaoEl('lookSent');btn.hide();}
if ( !window.Element )
{
        Element = function(){}
...
function displayImg(block){
    ...
    var blockImg = youdaoEl(block + "_right");
	if (blockImg) {
...
function loadingNext() {
    youdaoEl("main").hide();
    youdaoEl("loading_tip").show();
}
function cancelLoading() {
    youdaoEl("loading_tip").hide();
    youdaoEl("main").show();
}
...
function changeBlock(hideId, showId) {
    youdaoEl(hideId).hide();
    youdaoEl(showId).show();
}
function showDefinition(block) {
	// 如果block这个id存在，并且有内容，我们只是将其变成hidden，如果不存在，则需要下载。
    var eleBlock = youdaoEl(block);
	if (eleBlock) {
        var blockBody = youdaoEl(block+"_body");
		displayImg(block);
		if (blockBody) {
			blockBody.show();
		} else {
            ...
            youdaoEl(block).innerHTML = ("<p align=center><img align='absmiddle' src='" + image_path + "loading.gif' width='35'/><b>加载中...</b></p>");
			HTTP.getText(proxy_server_url + "dp?block=" + block + additionalParam + "&q=" + prefixQuery + encodeURIComponent(global_q) + suffixQuery + global_params, function(transport) {
                youdaoEl(block).innerHTML = transport;
			}
			);
		}
	}
...
function loadMoreLj() {
    var moreSentenseDiv = youdaoEl("more_sentence");
    youdaoEl("more_lj").innerHTML = ("<img align='absmiddle' src='" + image_path + "loading.gif' width='35'><b>更多例句加载中...</b>");
...
function loadMoreBlock(divName, tipTagName, blockName, additionalParam, prefixQuery, suffixQuery, loadingString) {
    ...
    var moreContentDiv = youdaoEl(divName);
    youdaoEl(tipTagName).innerHTML = ("<img align='absmiddle' src='" + image_path + "loading.gif' width='35'><b>"+loadingString+"</b>");
...
function showAll() {
    var elems = getBlocks(youdaoEl("main"));
	if (elems.length > 0) {
...
function changeBlock(block) {
	...
	if (blockBody) {
		blockBody.show();
        var blockTab = youdaoEl(block+"_tab");
		if (blockTab) {
				...
				for (var i = 0; i < children.length; i++) {
					if (children[i].nodeType == document.ELEMENT_NODE) {
						...
						if (children[i].tagName == "DIV" && indexBlockName > 0) {
							...
							if (childName != block) {
								children[i].hide();
                                var childTab = youdaoEl(childName+"_tab");
								if (childTab) {
									childTab.className = "";
								}
							}
...
function showMoreResult(block) {
    var blockBodyPart = youdaoEl(block+"_body_part");
    if (blockBodyPart) {
        blockBodyPart.hide();
    }
    
    var blockBodyFull = youdaoEl(block+"_body_full");
    if (blockBodyFull) {
        blockBodyFull.show();
    }
}
