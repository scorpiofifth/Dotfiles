...
complete -c pico -s p -l preserve -d 'Preserve the XOFF and XON sequences (^S and ^Q) so that they are caught by th…'
...
complete -c pico -s z -l listsyntaxes -d 'List the names of the available syntaxes and exit'
complete -c pico -l magic -d 'When neither the file\'s name nor its first line give a clue, try using libmag…'
complete -c pico -s @ -l colonparsing -d 'When a filename given on the command line ends in a colon plus digits and thi…'
...
complete -c pico -s / -l modernbindings -d 'Use key bindings similar to the ones that most modern programs use: ^X cuts, …'
